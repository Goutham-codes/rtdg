...
main_frame_1 = Frame(main_frame,width=730, height=340, bd=1, relief="ridge",bg='black')
...
def insertdata():
	global treecount
	if field_entry.get()=="" or len(field_entry.get())>20 or domain_var.get()=="" or subdomain_var=="":
		messagebox.showwarning('Invalid input','Check the input fields!')
		return
	if not domain_var.get() in all_func:
		messagebox.showwarning('Invalid input','Check the input fields!')
		return
	...
	tree.insert(parent='',index='end',iid=treecount,values=(treecount,field_entry.get(),domain_type))
	treecount+=1
	clearselection()
...
def openfile():
	global filename
	global filelabel
	filename = filedialog.asksaveasfilename(title='select a file',filetypes=(('csv files','*.csv'),('json files','*.json')))
	if filelabel!="":
		filelabel.destroy()
	filelabel=Label(bottom_frame,text=filename.split('/')[-1],font=('comicsans',10),fg='white',bg='black')
...
def getfileformat():
	...
	if not (len(namecheck)==2 and (namecheck[1].lower()=="csv" or namecheck[1].lower()=="json") and namecheck[0].isalnum()):
		return False
...
style.configure('Treeview',background='silver',foreground='black',rowheight=35,fieldbackground='silver')
...
tree.column('Id',width=240,minwidth=240,anchor=CENTER,stretch=False)
tree.column('Field',width=240,minwidth=240,anchor=CENTER,stretch=False)
tree.column('Type',width=240,minwidth=240,anchor=CENTER,stretch=False)
...
# title
Label(header_frame, text="AZGEDA RTDG",font=('comicsans',40,'bold'),fg='white',bg='black').place(relx=0.3,rely=0.3)
Button(header_frame,text='x',font=('comicsans',10),fg='white',bg='red',command=closeapp,width=3).place(relx=0.89,rely=0.45)


# main frame 2 widgets
Label(main_frame_2,text='Field Name',font=('comicsans',10),bg='black',fg='white').place(relx=0.02,rely=0.3)

field_entry = Entry(main_frame_2,textvariable=field_var,font=('comicsans',10))
...
subdomain_choosen = ttk.Combobox(main_frame_2, width = 15, textvariable = subdomain_var)
subdomain_choosen['values'] = ()
subdomain_choosen.place(relx=0.56,rely=0.3)

add_field_button = Button(main_frame_2,text='Add Field',font=('comicsans',10),command = insertdata,width=15)
add_field_button.place(relx=0.8,rely=0.3)


# main frame 3 widgets
Label(main_frame_3,text='Delete Field',font=('comicsans',10),bg='black',fg='white').place(relx=0.02,rely=0.3)
delete_entry = Entry(main_frame_3,textvariable=delete_var,font=('comicsans',10),width=10)
delete_entry.place(relx=0.14,rely=0.33)
delete_button = Button(main_frame_3,text='Delete',font=('comicsans',10),command = deletedata,width=15)
delete_button.place(relx=0.8,rely=0.3)


# bottom frame
Label(bottom_frame,text='Rows ',font=('comicsans',10),bg='black',fg='white').place(relx=0.04,rely=0.3)
row_entry = Entry(bottom_frame,textvariable=rows_var,font=('comicsans',10),width=10)
row_entry.place(relx=0.14,rely=0.33)
rows_var.set('100')
choose_file_button = Button(bottom_frame,text='choose file',font=('comicsans',10),command = openfile)
choose_file_button.place(relx=0.36,rely=0.3)
save_button = Button(bottom_frame,text='Save to file',font=('comicsans',10),command = download,width=15)
