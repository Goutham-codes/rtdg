'''
File : cli.py
Function : creates a command line interface and
...
def checkinputs(fields, total, filename):
    ...
    if not(len(namecheck) == 2 and namecheck[1] == "csv"):
        raise ValueError("argument 'filename' must be a valid csv file name")
    if not namecheck[0].isalnum():
        raise ValueError("argument 'filename' must be a valid csv file name")
    return True
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        prog='rtdg', description='test data generation')
